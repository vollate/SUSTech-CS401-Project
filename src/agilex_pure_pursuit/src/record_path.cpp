...
int main(int argc, char **argv) {
  ...
  ros::Publisher path_pub =
      nh.advertise<nav_msgs::Path>("target_path", 1, true);

  std::string path_file;
  private_nh.param("path_file", path_file,
                   std::string("/home/agilex/target_path.txt"));
  ...
  for (int i = 0; i < path_msg.poses.size(); ++i) {
    stream << path_msg.poses[i].pose.position.x << " "
           << path_msg.poses[i].pose.position.y << std::endl;
  }
