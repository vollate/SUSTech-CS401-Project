...
#include <string>
 ...
 void PurePursuit::backUp() {
    ...
    void PurePursuit::goalStatusCallback(const actionlib_msgs::GoalStatusArray::ConstPtr& status) {
...
//     double lookahead_distance = 1.0; // This should be dynamically adjusted based on speed or other factors

//     // Calculate the angle between the robot's current pose and the target point
//     double angle_to_target = atan2(p.position(1) - cur_pose_(1), p.position(0) - cur_pose_(0));
//     double heading_error = normalizeAngle(angle_to_target - cur_pose_(2));

//     // Calculate steering angle using the bicycle model
//     // steering_angle = atan((2 * wheelbase * sin(heading_error)) / lookahead_distance);
//     double steering_angle = atan2(2 * wheelbase_ * sin(heading_error), lookahead_distance);

//     // Limit the steering angle to vehicle's max steering angle
//     double max_steering_angle = M_PI / 6; // Example: max 30 degrees steering angle
...
PurePursuit::PurePursuit()
    ...
    // cmd_vel_pub_ = nh.advertise<ackermann_msgs/AckermannDriveStamped>(cmd_vel_topic, 1);
    ...
    status_sub = nh.subscribe("move_base/status", 1, &PurePursuit::goalStatusCallback,this);

    scan_sub_ = nh.subscribe(scan_topic, 1, &PurePursuit::scanCallback, this);

    cross_sub_ = nh.subscribe("detector/cross/result", 1, &PurePursuit::crossCallback, this);
    signal_sub_ = nh.subscribe("detector/signal/result", 1, &PurePursuit::signalCallback, this);
    traffic_light_sub_ = nh.subscribe("detector/light/result", 1, &PurePursuit::trafficLightCallback, this);
...
void PurePursuit::publishNewGoal(const WayPoint& new_target) {
    ...
    new_goal.pose.orientation = tf::createQuaternionMsgFromYaw(new_target.heading); // 设置正确的朝向

    ROS_INFO("publishing [%f, %f]", new_target.position(0), new_target.position(1));
